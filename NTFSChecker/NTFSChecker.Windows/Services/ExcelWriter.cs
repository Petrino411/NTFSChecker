...
public class ExcelWriter
{
    ...
    public ExcelWriter(ILogger<ExcelWriter> logger, SettingsService settingsService)
    {
        _logger = logger;
                    ...
                    if (isGroupInMainDir)
                    {
                        ...
                        else if (mainDirAccessUsers.Any(mu => mu[3] == accessUser[3]))
                        {
                            cell.Style.Font.Color.SetColor(
                                FromHtml(_settingsService.GetString("AppSettings:RightsColor"))); // Отличие в наборе прав
                        }
                        else
                        {
                            cell.Style.Font.Color.SetColor(
