        ...
        public async Task CreateLegendAsync()
        {
            ...
            var legends = new List<(string text, Color color)>
            {
                ("Группы пользователей нет у корневого каталога", 
                    FromHtml(App.Configuration["AppSettings:MainDirColor"]!)),
                ("Группы пользователей нет у дочернего каталога", 
                    FromHtml(App.Configuration["AppSettings:CurDirColor"]!)),
                ("Отличия в правах", 
                    FromHtml(App.Configuration["AppSettings:RightsColor"]!)),
                        ...
                        if (isGroupInMainDir)
                        {
                            ...
                            else if (mainDirAccessUsers.Any(mu => mu[3] == accessUser[3]))
                            {
                                cell.Style.Font.Color.SetColor(
                                    FromHtml(App.Configuration["AppSettings:RightsColor"]!)); // Отличие в наборе прав
                            }
                            else
                            {
                                cell.Style.Font.Color.SetColor(
                                    FromHtml(App.Configuration["AppSettings:RightsColor"]!)); // Отличие в типе прав
                            }
                        }
                        else
                        {
                            cell.Style.Font.Color.SetColor(
                                FromHtml(App.Configuration["AppSettings:MainDirColor"]!)); // Нет в корневом
                        }
                    }
                }
            ...
            foreach (var mainDirAccessUser in mainDirAccessUsers)
            {
                if (!accessUsers.Any(x => x.SequenceEqual(mainDirAccessUser)))
                {
                    for (var i = 0; i < mainDirAccessUser.Count; i++)
                    {
                        ...
                        cell.Style.VerticalAlignment = ExcelVerticalAlignment.Top;
                        cell.Style.Font.Color.SetColor(
                            FromHtml(App.Configuration["CurDirColor"]!)); // Нет в дочернем
                    }
